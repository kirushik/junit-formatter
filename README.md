...
`JUnitFormatter` accepts 3 options that can be passed in config.exs (or equivalent environment configuration for tests):
...
- `umbrella_relative_path` (boolean - default `true`): if set to `false`, Junit-formatter won't join `report_file` path with `Mix.project.app_path`; in case of `true` the paths are joined.
